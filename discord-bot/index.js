...
function initPlayerStats(userId, username) {
    ...
    if (!existing) {
        ...
        const update = db.prepare('UPDATE player_stats SET username = ? WHERE user_id = ?');
...
async function handleStartGames(message) {
    ...
    if (gameRunning || recruitingPlayers || joinTimer || joinedPlayers.size > 0) {
        await message.reply('The Noita Games are already in progress or recruiting players. Use `.stopgames` to cancel it first.');
        return;
    }
    ...
    await message.reply('The games have been announced! Waiting for tributes to join...');
...
async function startGame() {
    if (joinedPlayers.size < config.minPlayers) {
        ...
        const cancelEmbed = new EmbedBuilder()
            ...
            .setDescription(`Not enough tributes joined. Need at least ${config.minPlayers} players.`)
            .setColor(0xe74c3c);
    ...
    currentGame = new NoitaGame(eventsData, itemsData, materialsData, participants);
...
async function showResults(arenaChannel) {
    ...
    const resultsEmbed = new EmbedBuilder()
        ...
        .addFields({ name: 'Top Placements', value: placementText || 'No placements' })
        .setColor(0xf1c40f)
        .setTimestamp();
...
async function handleJoin(message) {
    ...
    if (gameRunning) {
        await message.reply('The Noita Games are already in progress. Wait for it to finish!');
        return;
    }

    if (!recruitingPlayers) {
        await message.reply("The Noita Games are not currently recruiting tributes, wait for the next games to be announced!");
        return;
    }
    ...
    if (arenaChannel) {
        const arenaEmbed = new EmbedBuilder()
            .setDescription(`**${message.author.displayName || message.author.username}** has volunteered as a tribute! (${joinedPlayers.size} tribute${joinedPlayers.size !== 1 ? 's' : ''} ${joinedPlayers.size !== 1 ? 'have' : 'has'} joined)`)
            .setColor(0x2ecc71);
...
async function handlePlayerList(message) {
    ...
    const listEmbed = new EmbedBuilder()
        ...
        .setFooter({ text: `${joinedPlayers.size} tribute${joinedPlayers.size !== 1 ? 's' : ''}` })
        .setColor(0x3498db);
...
async function handleAddFake(message) {
    ...
    if (!recruitingPlayers) {
        await message.reply("No game is currently recruiting. A moderator must use '.startgames' first.");
        return;
    }
    ...
    await message.reply(`Added ${addedCount} fake witches! Total tributes: ${joinedPlayers.size}`);
