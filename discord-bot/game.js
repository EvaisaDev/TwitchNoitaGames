...
class NoitaGame {
    ...
    getPlacements() {
        ...
        const finalOrder = survivors.length === 1
            ? [survivors[0], ...eliminated.map(u => this.participants.find(p => p.username === u))]
            : eliminated.map(u => this.participants.find(p => p.username === u));
        ...
        switch (this.stage) {
            case 0:
                ...
                return {
                    ...
                    description: phaseData.description || 'As the witches stand on their podiums, the music machine sounds.',
    ...
    runPhase() {
        ...
        this.consecutiveNoDeaths = this.killedThisPhase.length === 0 
            ? this.consecutiveNoDeaths + 1 
            : 0;
    ...
    runEvents(evObj) {
        ...
        while (aliveSet.size > 0) {
            ...
            const useFatal = roll < factor && this.participants.filter(p => p.alive).length > 1;
    ...
    replacePlaceholders(txt, killsCount) {
        ...
        for (const cat of allCategories) {
            ...
            if (cat === "weapon_any") {
                ...
                list = (this.itemsData[cat] || []).filter(w => w.max_kills >= killsCount);
