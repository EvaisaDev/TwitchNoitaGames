var eventHandlers = {
    polymorph: {
        ...
        getDisplayMention: function(username, gameState) {
            ...
            const newFormMention = polymorph.newFormId && !polymorph.newFormId.startsWith('fake_') ? `<@${polymorph.newFormId}>` : polymorph.newForm;
            const originalMention = polymorph.originalId && !polymorph.originalId.startsWith('fake_') ? `<@${polymorph.originalId}>` : polymorph.originalName;
        ...
        getWinnerMessage: function(username, gameState) {
            ...
            if (polymorph.newFormId && polymorph.originalId) {
                newFormDisplay = !polymorph.newFormId.startsWith('fake_') ? `<@${polymorph.newFormId}>` : polymorph.newForm;
                originalDisplay = !polymorph.originalId.startsWith('fake_') ? `<@${polymorph.originalId}>` : polymorph.originalName;
            } else {
